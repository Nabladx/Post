...
def getPosts(db: Session = Depends(database.get_db), payload: schema.TokenData = Depends(oauth2.validateAccessToken),
    ...
    posts = db.query(models.Post, func.count(models.Vote.postId).label("votes")).join(
        # func.count used to make a group by count
        models.Vote, models.Vote.postId == models.Post.id, isouter=True).group_by(models.Post.id).filter(
        # isouter ie. make it a LEFT OUTER JOIN
...
@router.post("/", status_code=status.HTTP_201_CREATED,
             response_model=schema.PostResponse)  # uses the response schema class
def createPost(post: schema.PostCreate, db: Session = Depends(database.get_db),
               payload: schema.TokenData = Depends(oauth2.validateAccessToken)):
    # fetch user data
    ...
    newPost = models.Post(userId=payload.userId,
                          **post.dict())  # creates a new instance and initializes it through the default constructor: https://stackoverflow.com/questions/15081542/python-creating-objects
    # alternate approacth
...
def getPost(id: int, db: Session = Depends(database.get_db),
            payload: schema.TokenData = Depends(oauth2.validateAccessToken)):
    post = db.query(models.Post, func.count(models.Vote.postId).label("votes")).join(
        # func.count used to make a group by count
        models.Vote, models.Vote.postId == models.Post.id, isouter=True).group_by(models.Post.id).filter(
